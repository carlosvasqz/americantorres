<?php
    ...
    function utilidadMes($fecha){
        include ('bd/conexion.php');
        $queryUtilidadMes=mysqli_query($db, "SELECT Utilidad FROM cierres_mensuales WHERE Fecha_Inicial LIKE '$fecha%'") or die(mysqli_error());
    ...
    function gastosMes($fecha){
        include ('bd/conexion.php');
        $queryGastosMes=mysqli_query($db, "SELECT (Total_Serv_Pub+Total_Planilla) AS Gastos FROM cierres_mensuales WHERE Fecha_Inicial LIKE '$fecha%'") or die(mysqli_error());
